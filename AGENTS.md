...
| 日期 | 变更内容 |
|------|---------|
| 2026-06-22 | 修复(用户反馈"默认应是干净2D，别默认3D")：厚度挤出层之前在平面模式也渲染(地图底部有笨重立体边)。现把厚度三件套包进 `g.map3d`，CSS 默认 `display:none`，仅 `#mapsvg-holder.tilt .map3d` 显示——**平面=干净2D，点🏔立体沙盘才出现厚度**。测试:平面 map3d none、立体 inline、可还原 |
| 2026-06-22 | ①地图足迹改**流动虚线(蚂蚁线)**表方向(`stroke-dashoffset` 动画 0.9s)，删除碍眼的"足迹（①→②顺序）"文字注解；编号停靠点保留并加**起点绿/终点赭**配色图形化标起止。②确立新项目规则「**图形优先于文字**」写入 `CLAUDE.md`(能图形化就不用文字，复杂逻辑才图形+文字；面向小学生/一般用户重图像轻阅读)，并记 memory `feedback-graphics-over-text`。测试:polyline 含 animate、顺序文字已消、5停靠点起点#3f7a52、控制台0报错 |
| 2026-06-22 | 立体沙盘增强真实3D感(用户反馈"倾斜后还是偏2D像斜放的纸")：根因是只倾斜了平面、地形无厚度。**给陆块做出厚度**——SVG内把 PROVPATH 轮廓向下错位叠15层(浅土黄→深褐"岩层侧壁")+底缘描边+加深落地投影，倾斜后成"悬浮立体玉版/沙盘块"而非斜纸。测试:15挤出层渲染、倾斜matrix3d、控制台0报错 |
| 2026-06-22 | 立体沙盘开关移到地图右下角悬浮按钮区(🏔 与 ＋/－/复位 一摞，置顶)，从拥挤的控件栏移出，更易发现；toggleMapTilt 改切换悬浮按钮高亮(不再 renderMapControls)。测试:悬浮区有🏔、控件栏已无、点击切换立体+高亮+提示、可还原 |
