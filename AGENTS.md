...
- 知识图谱 2D 力导向 + 伪 3D（**单击看信息卡 / 双击展开节点1-hop邻居·再双击折叠，累积式图数据库交互，封顶12+重置+种子保护**）+ 聚焦总览(朝代+诗人)；时间轴防重叠分层
- **诗词地图**（对标参考"诗词图卷"）：A1 缩放平移 / A2 真实晕渲地形(内嵌栅格染宣纸色)+山脉名 / A3 地形⇄素雅 / A4 古⇄今地名 / B1 名胜分类图标(山水楼关岳) / B2 足迹编号①②③ / 地点 hover 高亮 + 点空白关闭 tip
- **地图按诗人展开 + 清晰度**：顶部「诗人：全部 + 22 位」横滑选择行(`map-poets`)，选中某诗人→左侧列表只看其诗 + 顶部诗人简介头(印章/姓名/朝代生卒/入选数/×关闭)、地图标记只留其诗作地、有足迹则画。**清晰度**：标记/地名/足迹/名胜/山脉名缩放时反向缩放(`.zm` data-br / `.zt` data-bfs，`applyMarkScale` 在 `applyZoom` 调用)保持恒定屏幕尺寸不糊不重叠，描边用 `vector-effect=non-scaling-stroke`；滚轮缩放需 ⌘/Ctrl。 **水墨沙盘改造(2026-06-22)**：去右侧引线长列→地名按重要度精选前15+余者悬停浮标(`map-hover-label`)；墨珠地标(`beadG`渐变+`.zme`落地椭圆影+`mksh`阴影)；陆地`landsh`模糊投影+`skyG`绢色大气；「🏔立体沙盘」`toggleMapTilt`(`#mapsvg-holder.tilt` CSS3D rotateX，开启暂停缩放`mapTilt`守卫、地标仍可点)。
...
| 日期 | 变更内容 |
|------|---------|
| 2026-06-22 | **诗词地图水墨沙盘改造**(用户反馈"不清晰/不漂亮/能否立体")：①**清晰化**——删除旧"右侧海域引线长列"(主要杂乱源)，地名改为按重要度(诗数+传播度)精选前15(选诗人时显其全部地点)，余者**悬停浮标**显示；标签加白色描边halo提升在地形上的可读性。②**立体观感**(SVG内，不破坏缩放/点击)——墨珠地标(径向渐变球感+落地椭圆影+drop-shadow)、陆地投影(PROVPATH模糊偏移→悬浮沙盘感)、绢色径向大气背景。③**「🏔 立体沙盘」开关**——CSS3D `perspective rotateX(46deg)` 倾斜鸟瞰成手卷/沙盘；开启时暂停滚轮/拖动缩放(svgPt在3D下失真)、地标仍可点(直接监听)，关闭恢复。applyMarkScale 增椭圆反向缩放。测试:61墨珠+落地影、陆地投影、绢色大气、右侧引线列已消(rightColGone)、倾斜matrix3d生效+提示切换+可还原、控制台0报错 |
| 2026-06-22 | 修复图谱回归(用户反馈"单击诗人进入聚焦"消失)：双击展开功能误把单击改成只弹信息卡。现**单击=进入诗人/朝代聚焦(老交互恢复)+双击=展开1-hop(新功能)并存**，桌面用 230ms 点击延时区分(双击时 clearTimeout 取消挂起单击)，触屏 320ms 双击检测；focus-bar 恢复"←返回总览/正在看"且提示双击展开。测试：总览双击杜甫→focus仍null+77→89(展开未聚焦)✅、单击诗人→进入聚焦+8维度芯片✅、控制台0报错 |
| 2026-06-21 | 知识图谱新增**图数据库式双击展开1-hop**（用户选定累积式·真·图数据库交互）：单击节点=信息卡，**双击=把该点1-hop邻居加到画布**（诗人→其诗作 / 诗→朝代地点情感意象风格历史事件 / 属性节点→相关诗），**再双击=折叠**（仅删度数1的非种子邻居）。护栏：每次展开封顶`EXPAND_CAP=12`、力导向`gAlpha=1`重新冷却定格、"🔄重置为总览"清空、总览节点标记`_seed`永不被折叠删除；触屏320ms双击检测。focus-bar 改为操作提示+`flashGBar`展开/折叠反馈。新增 `nodeNeighbors/expandNode/collapseNode/flashGBar`，复用 `relatedPoemsFor`。测试：总览77种子→双击李白+12(13首封顶)→双击诗+8属性(6类全)→折叠删11(被二次展开的诗因增边保留)→重置回77；真实dblclick事件路径通过、控制台0报错 |
