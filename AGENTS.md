...
## 当前状态（2026-06-24）
- **朗读跨平台兜底（新）**：国产安卓(小米等)Chrome 常无 TTS 引擎→`speechSynthesis` 不发声。方案：有系统语音时仍用 TTS(逐句高亮);**无音色/不发声→自动回退内嵌音频 `RECITE_AUDIO`**(57首预渲染 mp3，整首播+整体高亮)，看门狗1.6s无声即切。生成：`scripts`思路用 macOS `say -v Tingting -r90`→ffmpeg 24k单声道→base64。覆盖：小学1-3全部+初中精选(共57)。文件涨到 ~4.1MB(仍在≤10MB预算)。**新增更多内嵌只需跑生成脚本扩 `RECITE_AUDIO`**。
